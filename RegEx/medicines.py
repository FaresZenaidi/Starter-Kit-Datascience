...
# url = ''
# Reg ex sur dataframe, df[''].str.extract('') / df[''].str.split('')
# Renommer colonne df -> df.rename
# requests.post()
# find_all command, \s: espace
# reg_ex = IBUPROFENE\s([\w\s]+)\s(\d+)\s?([a-zA-Z%+], ([\w]
# df[''].ix[--]
# .apply(fonction) -> appliquer fonction à chaque élément du df
